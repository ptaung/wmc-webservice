...
$url_process = Url::to(['/ws/process/run']);
$url = Url::to(['import']);
$url_count = Url::to(['countfile']);
$url_sync = Url::to(['sync']);
$url_dlc = Url::to(['dlc']);
$script = <<<JS

$("document").ready(function(){
    $("#runprocrss").click(function(){
           $("#runprocrss_show").html('<i class="fa fa-spinner fa-spin" aria-hidden="true"></i>');
           $.post("{$url_process}",function(data){
               $("#runprocrss_show").html('');
           });
     });
...
$js = <<<JS
function refresh() {
         $.ajax({ url: "$url_count", success: function(data) { $("#countFiletxt").html(data);}})
     setTimeout(refresh, 3000); // restart the function every 5 seconds
    }
refresh();
function refresh1() {
         $.ajax({ url: "$url", success: function(data) { $("#importprocess").html(data);}})
     setTimeout(refresh1, 3000); // restart the function every 5 seconds
    }
refresh1();
function refresh2() {
...
<div class="box box-warning">
    <div class="box-header with-border">
        <i class="fa fa-envelope " style="color: #F29829;"></i> <b>ระบบติดตามการนำเข้าข้อมูล <span id="countFiletxt"></span> <span class='btn btn-success btn-sm' id="runprocrss">&nbsp;RUN PROCESS&nbsp;</span>  <span id="runprocrss_show"></span></b>
    </div>
    <!-- /.box-header -->
    <div class="box-body" >
        ...
        [
            ...
            'value' => function($data) {
                return "<u>IP:{$data['hbs_ip']}</u>" .
                        '<br><small><b>CT:</b> ' . ($data['hbs_time'] <> '0000-00-00 00:00:00' ? $data['lastonline'] : '') . '</small>';
            }
        ],
        [
            'label' => 'START/FINISH',
            ...
            'format' => 'raw',
            'value' => function($data) {
                return '<small><b>ST:</b> ' . ($data['hbs_sync_start'] <> '0000-00-00 00:00:00' ? $data['synctime'] : '') . '</small>'
                        . '<br><small><b>FT:</b> ' . ($data['hbs_sync_finish'] <> '0000-00-00 00:00:00' ? $data['lastsync'] : '') . '</small>';
            }
        ],
        [
            'label' => 'UPLOAD',
