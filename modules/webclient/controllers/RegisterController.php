...
class RegisterController extends Controller {
    ...
    public function actionThaicvdrisksummary() {
        ...
        $query = "SELECT
            ...
            #dmht,
            t_mix_dx,
            ifnull(t.waist_cm,0) AS waist,
            t.smoking,
            ifnull(t.height,0) AS height,
            ifnull(t.rs_bps1,0) AS bps,
            ifnull(t.rs_tc,0) AS tc,
            t.sex,
            cvdrisk_color AS result
            ,IF(t.rs_tc < 1,IF(t.waist_cm > (t.height / 2) AND t.waist_cm > 0 AND t.height > 0, 'M',IF(t.waist_cm < (t.height / 2) AND t.waist_cm > 0 AND t.height > 0, 'L',IF(t.waist_cm > 0 AND t.height > 0, 'N',''))), 'N') AS wh_2
            ,IF(t.age_y>=65,65,IF(t.age_y>=60,60,IF(t.age_y>=55,55,IF(t.age_y>=50,50,40)))) AS age
            ,IF(t.type_dx IN ('02','03'),'Y','N') AS chronic
            ,IF(t.smoking = 1,'Y','N') AS smoke
            ,IF(t.rs_tc > 0 ,'Y','N') AS has
            ,IF(t.rs_tc >= 320, 320, IF(t.rs_tc >= 280, 280, IF(t.rs_tc >= 240, 240, IF(t.rs_tc >= 200, 200,IF(t.rs_tc < 200 && t.rs_tc > 0,160,0))))) AS cholesterol
            ,IF(t.rs_bps1 >= 180, 180, IF(t.rs_bps1 >= 160, 160, IF(t.rs_bps1 >= 140, 140,IF(t.rs_bps1 < 140 && t.rs_bps1 > 0,120,0)))) AS bp

    FROM
        t_cvd_screen t,t_person_cid p
    WHERE  (complication NOT IN (1,2) OR  complication IS NULL)
       #t.hospcode = '{$hoscode}'
           and p.cid = t.cid
{areacode}
{chronic}
        ...
        $mapKeyword = [
            '{table}' => 'dw_' . $hoscode . '.',
            '{areacode}' => " AND check_vhid LIKE '{$q['q_ampurcode']}%' ",
    ...
    FROM
        t_cvd_screen t,t_person_cid p
    WHERE t.hospcode = '{$hoscode}'
        AND (complication NOT IN (1,2) OR  complication IS NULL)
           AND p.cid = t.cid  {chronic}";
        $mapKeyword = [
            #'{table}' => 'dw_' . $hoscode . '.',
...
FROM
    ...
    public function actionDmhtscreen() {
        ...
        $query = "SELECT
                    ...
                    ,house_regist_type_id
                    ,nationality
                    ,CASE WHEN dmht='02' THEN 'เบาหวาน' WHEN dmht='01' THEN 'ความดัน' ELSE '-' END AS dmht
                    ...
                    ,last_fgc
                    FROM {table}person_dmht_screen_summary p1,{table}person_dmht_risk_screen_head p2
                    ,{table}person_ht_risk_bp_screen p3
                    ,{table}person_dm_fgc_screen p4
                    ...
                    LEFT JOIN (SELECT pid,t_mix_dx,type_dx AS dmht
                    FROM t_dmht WHERE hospcode = '{$hoscode}' AND type_dx IN ('01','02')) c ON c.pid = p.person_id
                    ...
                    (
                    SELECT pid FROM
                    (SELECT pid,t_mix_dx,type_dx AS dmht
                    FROM t_dmht WHERE hospcode = '{$hoscode}' AND type_dx IN ('01','02')
                    ) tmp WHERE dmht IN ('" . (!empty($q['q_screentype']) && $q['q_screentype'] == 1 ? "02" : "") . "" . (!empty($q['q_screentype']) && $q['q_screentype'] == 2 ? "01" : "") . "'))
...
";
    ...
    public function actionThaicvdloopupdetail() {
        ...
        foreach ($hospcode as $key => $hcode) {
            ...
            $table = 'dw_' . $hcode;
            $sqlQueryString = "SELECT '{$hcode}' as hospcode,o.vn,o.bps,o.vstdate,p.hn,o.waist,o.smoking_type_id as smoking,o.height,tc,
                timestampdiff(YEAR,birthday,o.vstdate) as ageatservice,
UNIX_TIMESTAMP(concat(o.vstdate,' ', o.vsttime)) as visittdate,pt.name as intype
                    FROM {$table}.opdscreen o
                            LEFT JOIN {$table}.ovst ov ON ov.vn = o.vn
                            LEFT JOIN {$table}.pttype pt ON pt.pttype = ov.pttype

                            LEFT JOIN {$table}.patient p ON p.hn = o.hn
                    WHERE  p.cid = '{$q['cid']}' and bps > 0 order by o.vstdate desc limit 30; ";
            try {
