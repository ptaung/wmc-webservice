...
class Report extends Controller {
    ...
    public $columns = [];
    public $sqlQuery = [];
    public $pageSize = 500; //จำนวนหน้าที่แสดงผล
    public $dataset;
    public $filter = [];
    ...
    public function strReplace($replaceMap = []) {
        ...
        $mapKeyword = [
            ...
            '{tmp}' => ($this->filter['filterTmp'] <> '' ? "='{$this->filter['filterTmp']}'" : NULL),
                #'{r506}' => ($this->filter['filterR506'] <> '' ? "='{$this->filter['filterR506']}'" : NULL), //filter โรคระบาด
        ];

        $this->filter['filterHospcode'] = (strlen($this->filter['filterHospcode']) == 5 ? "='{$this->filter['filterHospcode']}'" : NULL);
...
//แก้ไขคำสั่ง SQL ตาม filter
    ...
    public static function getMenuDetail($item) {
        $report = Cwebclient::getReportSerive($item);
        #$model = MenuItems::find()->where("menu_items_id ='{$report['item_id']}'")->one();
        return $report;
    }

    public static function getMenuGroupDetail($id) {
        if ($id === 0) {
            $model = new a;
        } else {
            $model = WuseGroupLocal::find()->where("menu_group_id ='{$id}'")->one();
        }
        return $model;
    }
